...
The following 15% of the announced quota | 50% scholarship`,
  ...
  const handleEditScholarship = (scholarship: Scholarship) => {
    setScholarships(
      scholarships.map((s) => (s.id === scholarship.id ? scholarship : s))
    );
  ...
  const handleDeleteScholarship = () => {
    if (currentScholarship) {
      setScholarships(
        scholarships.filter((s) => s.id !== currentScholarship.id)
      );
    ...
    <>
      ...
      <div className="space-y-4 ">
        {scholarships.map((scholarship) => (
          <div
            key={scholarship.id}
            className="bg-purple-50 p-4 rounded-md flex flex-col md:flex-row justify-between md:items-center gap-4"
          >
